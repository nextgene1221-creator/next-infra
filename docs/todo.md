...
1. タスクから科目がきえているか確認。タスク一覧に科目フィルターも残っているので関連する部分を広く確認すること
2. 講師と運営のダッシュボードに以下が追加されているか確認、なければ追加すること。今日使用するゼミプリントカード。生徒が自身で設定したゼミの予定日が今日のものだけ集計し、どの単元のＮｏが何枚必要なのか表示するもの
3．以下のイントロダクションを作成して。これまでのイントロダクションは一度すべて削除して。
    a. 講師用イントロダクション、導入にあたって。導入時のタスクについてまとめたもの。サインアップ、自身の生徒を招待、招待した生徒の大目標と週次目標を登録、ルーティンタスクの登録、学校イベントの登録
    b. 講師用イントロダクション、普段の運用について。
...
14. 講師が自身のルーティンタスクを `/tasks/routines` から設定できるように修正 (2026-05-09)
    - 原因: ルーティン管理ページが閲覧専用で、講師が自分のものを編集する導線がなかった（講師管理は admin 専用なので）
