...
13. 一時的なユーザー新規登録画面を作成 (2026-05-09)
    - 用途: システム導入のため、招待制ではなく塾に来た本人がその場で登録できる窓口。導入完了後に削除する想定
    - `/signup/student`, `/signup/teacher` の公開ページを新規追加（認証不要、`(authenticated)` の外側）
    - フィールドは admin 用の編集ページと同じ範囲（生徒は卒業年度・高校・保護者連絡先まで、講師は科目・電話・大学情報など）
    - パスワードはサーバー側で `password123` 固定。フォームには入力欄なし、登録完了画面でユーザーに通知
    - メール検証なし、フォーム送信 → 即作成 → 2 秒後にログイン画面へ遷移
    - `/api/signup/student`, `/api/signup/teacher` 公開 API。重複メールは 400 エラー
    - `/signup/layout.tsx` に「導入のため一時的に開放している窓口」の注意書きを表示
    - ログイン画面への導線追加なし（運営が直リンクを共有する想定）。**導入完了後はこのディレクトリ群を削除すれば招待制に戻る**
