...
1. 大目標や週次目標を追加するとき、開始日の初期値を今日に。週次目標の期日の初期入力値は開始日の７日後に、開始日が変更されたら変えるように。
2. 生徒側の大目標、週次目標が壊れている。ダッシュボードの大目標、週次目標カードが学習進捗で保存されたページ数を拾えていない。これはとても大きな問題なので、運営講師のほうでも共通に、大目標・週次目標・学習進捗の関係確認から仕様を細かく確認したい。
3. 生徒側のダッシュボードにある「今日進めるページ」の表示に関して、仕様を確認したい。
4. 自習室管理に、着席登録QRコードを表示を各校舎追加
5. ゼミ管理について、１の次に５番のプリントをやるといった飛ばしではなく、かならず1234といった連番で行う。なので予定登録の導線を変更するように。またマトリクスで空欄をクリックするとその単元の予定を登録できるようにしたい。
---
...
4. 生徒と講師を追加するとき、パスワードを講師や運営が手入力するのではなく、一律でpassword123にするよう統一 (2026-05-09)
