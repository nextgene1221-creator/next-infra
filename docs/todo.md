...
1. ゼミ管理で生徒側で操作した内容が講師側の画面で確認できなかったので確認して。
2. タスクの新規作成について、現在別画面で作成しているが、タイトルのみ入力して確定でとりあえず追加できるようにしたい。なので種別通常、期日を今日で固定し追加されるように。科目については削除すること。詳しく変更したい場合は修正から行うようにしたい。
...
10. 講師（および全ロール）の自身プロフィール表示画面 (2026-05-09)
