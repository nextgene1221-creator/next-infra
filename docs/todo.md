...
1. 講師の画面に自身のプロフィールを表示する画面を追加して。変更はできなくていい
2. ゼミ管理で生徒側で操作した内容が講師側の画面で確認できなかったので確認して。
3. タスクの新規作成について、現在別画面で作成しているが、タイトルのみ入力して確定でとりあえず追加できるようにしたい。なので種別通常、期日を今日で固定し追加されるように。科目については削除すること。詳しく変更したい場合は修正から行うようにしたい。
...
9. ゼミ管理を連番運用に + マトリクス空欄クリックで予定登録 (2026-05-09)
