...
1. 講師と運営のダッシュボードに以下が追加されているか確認、なければ追加すること。今日使用するゼミプリントカード。生徒が自身で設定したゼミの予定日が今日のものだけ集計し、どの単元のＮｏが何枚必要なのか表示するもの
2．以下のイントロダクションを作成して。これまでのイントロダクションは一度すべて削除して。
...
3. 自習室利用中の退室導線がない。ダッシュボードの自習室カードなどに追加すること。また着席登録の画面にも追加し、席移動（テーブルからカウンターに移動するなど）の導線も同じく着席登録の画面に追加すること
...
15. タスクと関連 RoutineTask から「科目」を完全削除 (2026-05-09)
