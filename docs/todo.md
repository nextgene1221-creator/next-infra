...
1. タスクの新規作成について、現在別画面で作成しているが、タイトルのみ入力して確定でとりあえず追加できるようにしたい。なので種別通常、期日を今日で固定し追加されるように。科目については削除すること。詳しく変更したい場合は修正から行うようにしたい。
...
11. ゼミ管理で生徒側操作が講師側で見えない問題の修正 (2026-05-09)
