...
1. ゼミ管理について、１の次に５番のプリントをやるといった飛ばしではなく、かならず1234といった連番で行う。なので予定登録の導線を変更するように。またマトリクスで空欄をクリックするとその単元の予定を登録できるようにしたい。
2. 講師の画面に自身のプロフィールを表示する画面を追加して。変更はできなくていい
3. ゼミ管理で生徒側で操作した内容が講師側の画面で確認できなかったので確認して。
4. タスクの新規作成について、現在別画面で作成しているが、タイトルのみ入力して確定でとりあえず追加できるようにしたい。なので種別通常、期日を今日で固定し追加されるように。科目については削除すること。詳しく変更したい場合は修正から行うようにしたい。
...
8. 自習室管理に、着席登録 QR コードを各校舎に追加 (2026-05-09)
