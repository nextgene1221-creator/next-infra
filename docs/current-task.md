...
## 2026-08-28 障害対応: 本番 DB の compute 枯渇

- 症状「システム内ログインに失敗」→ 実体は **Neon が HTTP 402 で全クエリを拒否**（compute time quota 超過）。
  DB を使う全機能が停止していた。詳細と教訓は playbook §9。
- **対応済み**: オーナーが Neon を有料プランへアップグレード → 復旧を確認（`select now()` / Prisma クエリ / 本番の認証ページ配信）。
  `MeetingAlertPoller` の間隔を 60 秒 → 5 分に緩和（commit `46f9a61`・本番デプロイ済み）。
- **未対応（オーナー判断待ち）**:
  - `/api/alerts/check-all` をクライアント発ポーリングから **Cron 側へ移す**（設計変更のため）
  - 生徒には `MeetingAlertPoller` を動かさない（生徒に不要なチェック）／タブ非表示時に止める
  - ローカル開発が本番 DB を直接叩く構成（Q7 の積み残し）
- ⚠️ **セキュリティ**: リポジトリは **Public**。`docs/understanding-2026-08-18.md` に
  テスト管理者の資格情報 (`test-admin@dev.local` / `test1234`) が書かれている。
  現在この資格情報は通らない（誰かが変更した可能性が高い）が、**公開リポジトリに載せる運用自体を止めるべき**。
