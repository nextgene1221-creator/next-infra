...
> **【実装追記 2026-08-24】交通費のカラムを追加（オーナー指示 2026-08-24）**
> マイグレーション `20260824000000_add_transport_allowance`（ADD COLUMN のみ・既存データ非破壊。本番 Neon 適用済み）。
>
> | テーブル | 追加カラム | 説明 |
> |---|---|---|
> | `users` | `transport_allowance_yen`（既定 200） | **出勤 1 日あたりの交通費**。人ごとに変更可。時給と同じく `User` に持たせる（管理者が `Teacher` を持たない場合があるため） |
> | `payslips` | `work_days` / `transport_yen` | 出勤日数と交通費の合計。明細生成時に確定させ、後から単価を変えても発行済み明細は動かない |
> | `payslip_items` | `transport_yen` | その日の交通費 |
...
> - **マスタの中身は開発側で流し込まない**（オーナー指示 2026-08-18）。**管理者が `/mock-exams` の画面から登録する**導線とする。開発用の一括投入スクリプトは廃止した。
> - **「未登録の模試名」セクション**: 既存の模試結果で使われているがマスタに無い名前を、**使用件数の多い順**に一覧表示する。管理者は中身を見ながら 1 件ずつ「登録」できる。
>   - 登録すると、**その名前と完全一致する既存の模試結果が自動でマスタに紐付く**（`mock_exam_id` を更新）。件数は登録前の確認ダイアログに表示する。
>   - 表記ゆれ（似ているが一致しない名前）は**自動では寄せない**。
> - **「表記ゆれの可能性」セクション**: 登録済み・未登録をまたいで、正規化キー（空白・記号・末尾の「模試」を除去）が一致する名前をグループ化して警告表示する。同じ模試なら 1 つだけ登録するよう促す。
...
> - **深夜・残業は自動計算しない**（B-9(c)）。必要な場合は明細ごとの `adjustment_yen`（手動調整・マイナス可）で吸収する。
> - **交通費は自動計算する**（2026-08-24 追加）。詳細は下の【実装追記 2026-08-24】。
...
> **【実装追記 2026-08-24】交通費の自動加算・UI 改修**
>
> **交通費（オーナー指示 2026-08-24）**
> - 講師・管理者の給与に、**出勤 1 日につき定額の交通費**を加算する。既定 **200 円**。
> - 単価は **人ごとに変更できる**（`users.transport_allowance_yen`）。変更できる画面は次の 2 つ:
>   - `/teachers/[id]/edit`（講師情報編集）… 「交通費（出勤1日あたり・円）」欄
>   - `/payroll` の「時給・交通費」モーダル … **講師・管理者の両方**を一覧から変更できる（管理者専用の編集画面が無いため、管理者はここが編集窓口）
> - 加算単位は **JST の日**。1 日に複数回の出退勤があっても **1 日 1 回だけ**付く。
> - **退勤打刻漏れ（0 分集計）の日も交通費は付く**。出勤した事実はあるため（0 分にするのは労働時間の話）。
> - 支給合計 = 勤務分 (`base_yen`) ＋ 交通費 (`transport_yen`) ＋ 手動調整 (`adjustment_yen`)。
> - 単価変更は履歴を持たない。**変更後に対象月の明細を「再生成」すると、その月全体が新単価で再計算される**。確定済み明細は再生成できないため過去分は動かない。
> - API: `PUT /api/payroll/allowance`（admin のみ / body: `userId`, `transportAllowanceYen`）。`POST`・`PUT /api/teachers/[id]` も `transportAllowanceYen` を受け付ける。
>
> **UI 改修（オーナー指示 2026-08-24）**
> - 時給の操作を**一覧の下に開くカードから モーダル（ポップアップ）へ変更**。どの行を編集しているのか分からない問題を解消。手当・控除の「調整」も同じくモーダル化した。
> - **一覧が即時反映されない不具合を修正**。原因は `PayrollManager` が props の `staff` を `useState` に写して固定していたため、`router.refresh()` でサーバー側が再計算されても画面が古いままだったこと。props を直接描画し、更新後は `useTransition` + `router.refresh()` で再取得する形にした。**「明細生成が機能しない」ように見えていたのも同じ原因**（明細自体は生成されていたが一覧に出なかった）。
> - 一覧に「交通費/日」列を追加。明細にも「交通費（N日）」の集計欄と日別の交通費列を追加。
> - **勤務が 16 時間を超える日を警告**するようにした（退勤打刻の押し忘れで金額が跳ねるのを検知するため）。
>
> **セキュリティ修正（同時対応）**
